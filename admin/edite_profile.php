...
if($_SERVER['REQUEST_METHOD']=="POST"){
	$req2="UPDATE  admin set username=?,adresse=?,email=?,nom_complet=?,photo=? where username='$id'";
	...
	if(isset($_FILES['photo']) && $_FILES['photo']['name'] != ""){
		...
		if(!move_uploaded_file($photo_tmp,"../icon/image/$photo_name")){
	...
	$stet->execute([$user,$adresse,$email,$name,$photo_name]);
					...
					<div class="card">
						...
						<form class="" action="" method="post" enctype="multipart/form-data">
							...
							<div class="row mb-3">
								...
								<div class="col-sm-9 text-secondary">
									<input  name="email"type="text" class="form-control" value="<?=$table1['email']?>">
								</div>
							</div>
