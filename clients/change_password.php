...
session_start();
$stet=$con->prepare("select * from clients ");
$stet->execute();
...
if(isset($_POST['curent_password']) && isset($_POST['new_password']) && isset($_POST['confirm_password'])){
    if( password_verify($_POST['curent_password'],$table['password'])){
        ...
        if(password_verify($_POST['confirm_password'],$hash)   ){
            $id=$_SESSION['user'];
            $stet2=$con->prepare("UPDATE clients set password='$hash' where username='$id' ");
            $stet2->execute();
    ...
    <div class="registration-form">
        ...
        <form method="post">
            ...
            <div class="form-icon text-center" >
                <img src="profile/<?=$table['photo']?>" style="width:120px;" class="rounded-circle p-1 bg-primary" alt="">
            </div>
